...
def base64_to_opencv(im_b64):
    """
    Auxiliary function to convert a base64 encoded string into an grayscale opencv image

    :param im_b64: base64 encode of an image
...
class FaceRecognition:
    ...
    def __get_faces_and_labels(self):
        ...
        the images stores in the database that belong to him, appending the images to the list 'faces' and the user id
        (which will be used as label) to the list 'ids'.
        ...
        if self.__conn:
            ...
            query = "SELECT image FROM IMAGE WHERE user_id = %s;"
            for user in users:
                cursor.execute(query, [user])
                for image in cursor.fetchall():
                    faces.append(base64_to_opencv(image[0]))
                    ids.append(user)
                cursor.reset()
            cursor.close()
    ...
    def __get_label(self, email):
        """
        Private function to retrieves a user id (used as a label in the LBPHFaceRecognition) from the database, based
        on the user email.
    ...
    def user_exists(self, email):
        """
        Verifies if the user already exists on the database, based on their email, by checking if an user with the
    ...
    def register(self, email, password, collected_images):
        ...
        Registers an user by first, hashing the provided password, and then inserting it on the database with the user's
        email. After retrieves the created user's id and with that stores all the encodings of the faces. After thath,
        updates the LBPHFaceRecognition so that the user can be authenticated.
        ...
        # TODO: encrypt collected_images ?
        image_query = "INSERT INTO IMAGE (image, user_id) VALUES (%s, %s)"
        values = [(face, user_id) for face in collected_images]
        cursor.executemany(image_query, values)
        self.__conn.commit()
        cursor.close()

        self.__recognizer.update(
            [base64_to_opencv(face) for face in collected_images],
    ...
    def authenticate(self, email, image):
        """
        Authenticate an user based on its email and face's image.

        Authenticates an user by first checking if the provided email is registered on the database, if so it retrieves
